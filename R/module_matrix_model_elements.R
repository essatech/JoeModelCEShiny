...
module_matrix_model_elements_ui <- function(id) {
  ...
  tagList(
    ...
    shinydashboard::box(
      ...
      tags$div(
        class = "lam_bb",
        actionButton(ns("model_component"), "Model Components")
      ),
                 ...
                 observeEvent(input$model_component, {
                   showModal(modalDialog(
                     title = "Model Component",
                       tagList(
                         tags$p("Under density-dependent growth conditions the projection matrix (A) is the product of the transition matrix (B), consisting of the life history charactersitcis, and the density-dependce matrix (D). A = B*D."),
